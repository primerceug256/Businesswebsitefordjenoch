  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center px-4">
      <div className="bg-gray-900 border border-gray-800 p-8 rounded-3xl w-full max-w-md shadow-2xl">
        ...
        <button onClick={loginWithGoogle} className="w-full bg-white text-black py-3 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-gray-100 mb-6">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-orange-600 text-white py-4 rounded-xl font-black uppercase hover:bg-orange-700 transition-all">{loading ? <Loader2 className="animate-spin mx-auto" /> : 'Login'}</button>
        </form>
        <p className="text-center mt-6 text-gray-500 text-sm font-bold">New here? <Link to="/signup" className="text-orange-500">Sign Up</Link></p>
      </div>
    </div>
  );
}