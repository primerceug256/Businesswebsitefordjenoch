  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col font-sans">
      <header className="bg-gradient-to-r from-orange-600 to-orange-500 sticky top-0 z-50 shadow-xl h-16 flex items-center px-4 justify-between">
          <Link to="/" className="text-xl font-black tracking-tighter">DJ ENOCH <span className="bg-black px-2 rounded">PRO UG</span></Link>
          ...
          <div className="fixed inset-0 top-16 bg-black z-50 p-6 overflow-y-auto animate-in slide-in-from-right">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              ...
              <div>
                ...
                <nav className="flex flex-col gap-4">
                  <MenuLink onClick={()=>setIsMenuOpen(false)} to="/dj-drops" icon={<Headphones/>} label="Order DJ Drop" sub="Custom Name Drop" iconColor="text-orange-500" />
                  <MenuLink onClick={()=>setIsMenuOpen(false)} to="/software" icon={<Download/>} label="Software" sub="Applications" />
                </nav>
              </div>
              <div>
            ...
            {user && <button onClick={()=>{logout(); setIsMenuOpen(false); navigate('/')}} className="mt-10 w-full py-4 bg-red-600/10 text-red-500 rounded-2xl font-black border border-red-600/20 uppercase">Logout ({user.email})</button>}
          </div>
        )}
      </header>
...
function MenuLink({ to, icon, label, sub, onClick, iconColor = "text-white" }: any) {
  return (
    <Link to={to} onClick={onClick} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-900 group">
      <div className={`p-3 bg-gray-800 rounded-xl group-hover:bg-orange-600 transition-all ${iconColor}`}>{icon}</div>
      <div><p className="font-black text-lg">{label}</p><p className="text-gray-500 text-xs">{sub}</p></div>
    </Link>
  );
}