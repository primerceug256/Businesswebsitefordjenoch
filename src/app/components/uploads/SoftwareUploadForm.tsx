...
import { projectId, publicAnonKey } from "../../../../utils/import { useState } from "react";
import { Upload, Package, Loader, CheckCircle } from "lucide-react";
...
export function SoftwareUploadForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="space-y-4">
      {success && <div className="bg-green-600/20 text-green-400 p-3 rounded-lg border border-green-600/30 font-bold">PUBLISHED!</div>}
      <form onSubmit={handleUpload} className="space-y-3 text-white">
        <div className="border-2 border-dashed border-slate-700 rounded-2xl p-6 text-center bg-slate-900/50">
            <input type="file" onChange={e => {
                const f = e.target.files?.[0];
                if(f) { setFile(f); setForm({...form, title: f.name.split('.')[0]}); }
            }} className="hidden" id="sw-file" />
            <label htmlFor="sw-file" className="cursor-pointer block">
                {file ? <p className="text-orange-400 font-bold">{file.name}</p> : <p className="text-xs uppercase font-black opacity-50">Select APK / EXE File</p>}
            </label>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <input placeholder="Price" value={form.price} onChange={e => setForm({...form, price: e.target.value})} className="bg-slate-950 p-3 rounded-xl border border-slate-800" />
        </div>

        {uploading && (
          <div className="w-full bg-slate-800 h-4 rounded-full overflow-hidden">
            <div className="bg-orange-500 h-full transition-all duration-300" style={{ width: `${prog}%` }}></div>
          </div>
        )}

        <button type="submit" disabled={uploading || !file} className="w-full bg-orange-600 py-4 rounded-xl font-black uppercase">
          {uploading ? `PROCESSING ${prog}%` : "PUBLISH NOW"}
        </button>
      </form>
    </div>
  );
