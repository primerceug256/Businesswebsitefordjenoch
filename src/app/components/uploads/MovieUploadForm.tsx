  ...
  return (
    <form onSubmit={handleUpload} className="space-y-4">
      <div className="p-6 border-2 border-dashed rounded-3xl text-center bg-gray-50">
        <input type="file" accept="video/*" onChange={e => setSelectedFile(e.target.files?.[0] || null)} className="hidden" id="mov-up-final" />
        <label htmlFor="mov-up-final" className="cursor-pointer block">
          <Film className="mx-auto mb-2 text-red-600 w-10 h-10" />
          <p className="font-bold text-gray-900">{selectedFile ? selectedFile.name : "Select Movie"}</p>
        </label>
      </div>
      <input placeholder="Movie Title" className="w-full p-4 bg-gray-100 rounded-2xl font-bold" value={title} onChange={e => setTitle(e.target.value)} required />
      <button disabled={uploading || !selectedFile} className="w-full bg-red-600 text-white py-4 rounded-xl font-black uppercase shadow-lg">
        {uploading ? `Uploading ${progress}%...` : "Publish to Cinema"}
      </button>
    </form>
  );
}